...
public:
    TaskList createTasks()
    {
        TaskList tasks = {
            ...
            TestHelpers::createTask(9300, QStringLiteral("Organization, Operations, Coordination"),
                                    8000),
...
private Q_SLOTS:
    void initTestCase() { m_model.setTasks(createTasks()); }
    ...
    void checkTaskIndex()
    {
        ...
        QVERIFY(index.data(TaskModel::FilterRole).toString()
                == QStringLiteral("9700 KDAB/Administration/HR Investments"));
        ...
        QVERIFY(index2.data(TaskModel::FilterRole).toString()
                == QStringLiteral("0021 KDAB/Customer Projects/Customer B/Project Y"));
    }

    void checkSmartName()
    {
        QVERIFY(m_model.smartTaskName(12)
                == QStringLiteral("0012 Customer A/Project X/Development"));
        QVERIFY(m_model.smartTaskName(21) == QStringLiteral("0021 Customer B/Project Y"));

        QVERIFY(m_model.smartTaskName(2003)
                == QStringLiteral("2003 Training is done by a trainer"));
        ...
        QVERIFY(m_model.smartTaskName(8304)
                == QStringLiteral("8304 Education/Education Time/Qt Contributions"));
