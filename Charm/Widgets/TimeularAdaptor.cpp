...
{
    QObject::connect(m_manager, &TimeularManager::orientationChanged, this, &TimeularAdaptor::faceChanged);
    QObject::connect(m_manager, &TimeularManager::statusChanged, this, &TimeularAdaptor::deviceStatusChanged);

    m_connectionAction = new QAction(QLatin1String("Connect to Device"), this);
    m_connectionAction->setCheckable(true);
    QObject::connect(m_connectionAction, &QAction::triggered, this, &TimeularAdaptor::toggleConnection);
...
void TimeularAdaptor::faceChanged(TimeularManager::Orientation face)
{
    ...
    for(const auto &m : m_faceMappings) {
        if(m.face == face && DATAMODEL->taskExists(m.taskId)) {
